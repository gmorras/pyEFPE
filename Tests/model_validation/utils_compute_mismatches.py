...
pyEFPE_keys = {'mass1': 'm1', 'mass2': 'm2', 'e_start': 'ecc', 'mean_anomaly_start':'mean_anomaly', 'spin1x': 's1x', 'spin1y': 's1y', 'spin1z': 's1z', 'spin2x': 's2x', 'spin2y': 's2y', 'spin2z': 's2z', 'inclination': 'iota', 'phi_start': 'phiref', 'f22_end': 'f_max'}
...
def generate_rand_params(Nsamples, mc_low=1.4, mc_high=2.6, q_low=0.25, q_high=1, s1_low=None, s1_high=1, s2_low=None, s2_high=1, ecc_low=0, ecc_high=0, precessing=True):
	...
	if precessing: s1_low, s2_low = 0, 0
	else:          s1_low, s2_low = -s1_high, -s2_high
...
def rotate_spins(phases, s1p, s2p):
		...
		if len(phases)==2:   R2 = R1
		#if three arguments are given, rotate spins independently
		elif len(phases)==3: R2 = rotation_matrix_2D(phases[2])
		else: raise Exception('Expected 2 or three arguments in local_mismatch_func')
...
def minimize_precessing_mismatch(signal, waveform, freqs, parameters, asd=None, Nph_tries=20, rotate_individual_spins=False, additional_minimize_threshold=1e-2, disp=False):
	...
	if rotate_individual_spins: Nphases = 3
	else:                       Nphases = 2
	phases_tries = np.random.uniform(0, 2*np.pi, size=(Nph_tries, Nphases))

	#generate mismatches for these ranom phases
	mismatches_tries = np.array([local_mismatch_func(phases_tries[i]) for i in range(Nph_tries)])
	...
	if min_mm > additional_minimize_threshold:
		...
		minimize_result = minimize(local_mismatch_func, phases_tries[i_min], method='Nelder-Mead', options={'disp': disp, 'fatol': additional_minimize_threshold})
	...
	min_mm_2, dt_shift, ang_sh, amp_sh, pol_h, = local_mismatch_func(min_phases, return_optimum_params=True)
	
	#compute the mismatch for each and minimize
	return min_mm, dt_shift, ang_sh, amp_sh, pol_h, *min_phases

#function to initialize waveforms, return functions that take a dictionary p with keys ['m1', 'm2', 's1x', 's1y', 's1z', 's2x', 's2y', 's2z', 'iota', 'phiref', 'pol', 'f_max'] and output a waveform
def initialize_pyEFPE_and_LAL_approxs(f_min, delta_f, params_pyEFPE, approx_string, pn_spin_order, pn_phase_order, pn_amplitude_order):
	
	#waveflags for LAL
	waveform_dictionary = lal.CreateDict()
	if pn_spin_order is not None: lalsim.SimInspiralWaveformParamsInsertPNSpinOrder(waveform_dictionary, int(pn_spin_order))
	if pn_phase_order is not None: lalsim.SimInspiralWaveformParamsInsertPNPhaseOrder(waveform_dictionary, int(pn_phase_order))
	if pn_amplitude_order is not None: lalsim.SimInspiralWaveformParamsInsertPNAmplitudeOrder(waveform_dictionary, int(pn_amplitude_order))
	
	#get approximant from string
	approx_lalsim = lalsim.SimInspiralGetApproximantFromString(approx_string)
	
	#consider the case the approximant is time-domain or frequency-domain
	...
	#LAL function that we will use
	def approx_LAL(params, phases=None):
		...
		if phases is not None:
			#update spin vectors with rotated versions
			...
			#update also the reference phase
			p['phiref'] = phases[0]
		
		#return polarizations from lal approximant
		return approx_func(p['m1'], p['m2'], p['s1x'], p['s1y'], p['s1z'], p['s2x'], p['s2y'], p['s2z'], params_pyEFPE['distance'], p['iota'], p['phiref'], 0, p['ecc'], p['mean_anomaly'], delta_f, f_min, p['f_max'], params_pyEFPE['f22_start'], waveform_dictionary, approx_lalsim)
		
	#put the params to pass to pyEFPE in dictionary
	def approx_pyEFPE(params):
		#put the params to pass to pyEFPE in dictionary
		p_pyEFPE = params_pyEFPE.copy()
		for pyEFPE_key, LAL_key in pyEFPE_keys.items(): p_pyEFPE[pyEFPE_key] = params[LAL_key]

		#compute frequency array
		freqs = np.arange(f_min, params['f_max'], delta_f)
	
		#Initialize pyEFPE waveform
		wf = pyEFPE.pyEFPE(p_pyEFPE)
		...
		#return the waveform, the frequency and the parameters
		return h_pyEFPE, freqs
	
	#return these functions we have created
	return approx_LAL, approx_pyEFPE
...
def generate_string_ID(Nsamples, precessing, approximant, psd_name, mc_low, mc_high, q_low, q_high, s1_high, s2_high, flim_type, pn_spin_order, pn_phase_order, params_pyEFPE, ecc_high=0, rotate_individual_spins=False, fmax_flim=1, string_start='_'):
	...
	string_ID += approximant+'_N_%s_mc_%.3g_%.3g_q_%.3g_%.3g_s1_%.3g_s2_%.3g_fmax_%.3g%s_PN_spin_%s_phase_%s'%(Nsamples, mc_low, mc_high, q_low, q_high, s1_high, s2_high, fmax_flim, flim_type, pn_spin_order, pn_phase_order)
	...
	if precessing: string_ID += '_ROTs1s2_%s'%(rotate_individual_spins)
...
def random_mismatches_with_EFPE(Nsamples, approx_string, seglen=128, f_min=20, flim_type='ISCO', fmax_flim=1, psd_name=None, distance_Mpc=10, mc_low=1.4, mc_high=2.6, q_low=1, q_high=4, s1_low=None, s1_high=1, s2_low=None, s2_high=1, ecc_high=0, precessing=True, params_pyEFPE={}, pn_spin_order=None, pn_phase_order=None, pn_amplitude_order=None, Nph_tries=20, rotate_individual_spins=False, additional_minimize_threshold=1e-2, outdir='./outdir', nworkers=-1, print_progress=True):
	...
	params.update({'approx_string': approx_string, 'psd_name': psd_name, 'dL': distance_Mpc, 'seglen': seglen, 'f_min': f_min, 'flim_type': flim_type, 'fmax_flim': fmax_flim, 'delta_f': delta_f, 'precessing': precessing, 'params_pyEFPE': params_pyEFPE, 'pn_spin_order': pn_spin_order, 'pn_phase_order': pn_phase_order, 'pn_amplitude_order': pn_amplitude_order, })
	...
	approx_LAL, approx_pyEFPE = initialize_pyEFPE_and_LAL_approxs(f_min, delta_f, params_pyEFPE, approx_string, pn_spin_order, pn_phase_order, pn_amplitude_order)
	...
	def mismatch_with_EFPE(p):
		...
		if precessing:
			output_mismatch = minimize_precessing_mismatch(h_pyEFPE, approx_LAL, freqs, p, asd=asd, Nph_tries=Nph_tries, rotate_individual_spins=rotate_individual_spins, additional_minimize_threshold=additional_minimize_threshold)
		else:
			hp_LAL, hc_LAL = approx_LAL(p)
			output_mismatch = mismatch_t_ph_amp_pol_minimized(h_pyEFPE, hp_LAL, hc_LAL, freqs, asd=asd, return_optimum_params=True)

		#if required print information
		if print_progress: print('%s/%s -> mc=%.3g, q=%.2f, chi_eff=%.2g, chi_p=%.2g -> MM: %.3g'%(p['ip']+1, Nsamples, params['mc'][p['ip']], params['q'][p['ip']], params['chi_eff'][p['ip']], params['chi_p'][p['ip']], output_mismatch[0]))
	...
	for ip, p in enumerate(param_dicts):
		...
		for pyEFPE_key, LAL_key in pyEFPE_keys.items(): all_params_pyEFPE[ip][pyEFPE_key] = p[LAL_key]
	...
	string_id = generate_string_ID(Nsamples, precessing, approx_string, psd_name, mc_low, mc_high, q_low, q_high, s1_high, s2_high, flim_type, pn_spin_order, pn_phase_order, params_pyEFPE, rotate_individual_spins=rotate_individual_spins, fmax_flim=fmax_flim, ecc_high=ecc_high)
...
def compare_waveforms_from_params(params, plot_order='maximum mismatch'):
	...
	#initialize the waveform generators
	approx_LAL, approx_pyEFPE = initialize_pyEFPE_and_LAL_approxs(params['f_min'], params['delta_f'], params['params_pyEFPE'], params['approx_string'], params['pn_spin_order'], params['pn_phase_order'], params['pn_amplitude_order'])
	...
	for ip in iplot:
		...
		#compute the LAL waveform
		if params['precessing']: hp_LAL, hc_LAL = approx_LAL(p, phases=params['phases_h'][ip])
		else:                    hp_LAL, hc_LAL = approx_LAL(p)

		h_LAL = pol_response(hp_LAL, hc_LAL, params['pol_h'][ip])
		
		#adjust the amplitude, phase and reference time of LAL waveform
		h_LAL = (h_LAL/params['amp_sh'][ip])*np.exp(-1j*(2*np.pi*freqs*params['dt_shift_sh'][ip] + params['ang_sh'][ip]))
		...
		title_str = r'$\mathcal{M}_c = %.3g M_\odot$, $q=%.2f$, $\chi_\mathrm{eff}=%.2g$, $\chi_\mathrm{p}=%.2g$ $\rightarrow$ MM: %.3g'%(params['mc'][ip], params['q'][ip], params['chi_eff'][ip], params['chi_p'][ip], params['mismatches'][ip])
		
		#print informations
		print('\nmc=%.3g, q=%.2f, chi_eff=%.2g, chi_p=%.2g -> MM: %.3g  MM_direct: %.3g'%(params['mc'][ip], params['q'][ip], params['chi_eff'][ip], params['chi_p'][ip], params['mismatches'][ip], mm_direct))
		
		#make a plot of h_pyEFPE vs h_LAL
		fig = plot_h1_h2(h_pyEFPE, h_LAL, freqs, asd=asd, label_1=r'$h_\mathrm{EFPE}$', label_2 =r'$h_\mathrm{%s}$'%(params['approx_string']), title_str=title_str)
...
def plot_h1_h2(h1, h2, freqs, asd=None, label_1=r'$h_1$', label_2=r'$h_2$', show=True, figsize=(12,12), title_str=None):
	...
	axs[1].plot(freqs, np.unwrap(np.angle(norm_prod), period=2*np.pi)/(2*np.pi), label=r'$\frac{\varphi(h_2) - \varphi(h_1)}{2 \pi}$')
	axs[1].legend()
	...
	#add title
	if title_str is not None: title_str += '\n'
	else: title_str = ''
	title_str += r'%s $\rightarrow$ $h_1$,  %s $\rightarrow$ $h_2$'%(label_1, label_2)
